...
if plotstuff
    ...
    using Colors
    category_colors = [
        [1, 0, 0, 0.0],  # Red with 50% transparency
        [0, 1, 0, 1.0],  # Green (fully opaque)
    ]
    ...
    category_colors = [
        [0, 0, 0, 0.0],  # green with 50% transparency
        [1, 0, 0, 1.0],  # red (fully opaque)
    ]
    fig = PyPlot.figure()
    ax = fig[:add_subplot](111, projection="3d")
...
function x2logKs(x)
    ...
    return cat(logKs..., dims=3)  # concatenate along the first dimension
end
