...
function solve_numerical(Qs, T)
    ...
    permVal=exp.(reshape(T,n*n))
    everystep=false # output all the time 
    args=h0, S0, permVal, dirichleths,  dirichletnodes, Qs, volumes, areasoverlengths, fluid, dt, neighbors, nt, everystep
    h_gw, S= DPFEHM.solvetwophase(args...)
    return h_gw[monitoring_well_node]- steadyhead
end

@BSON.load "mytrained_model.bson" model
θ = Flux.params(model)
...
for epoch in epochs
    ...
    @BSON.save "mytrained_model_Finetuned.bson" model
end
@BSON.save "model_Finetuned_$(batch_size)_$(learning_rate).bson"  epochs train_time_pt losses_train_pt  rmses_train_pt  losses_test_pt  rmses_test_pt  
