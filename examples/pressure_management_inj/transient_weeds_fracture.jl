...
function solveforh(logKs, dirichleths)
	...
	result = DifferentiableBackwardEuler.steps(h0, f_gw, f_gw_u, f_gw_p, f_gw_t, p, 0.0, 1.0 * 60 * 60 * 24 * 365 * 10; abstol=1e-1, reltol=1e-1)
    #plotit(result, logKs)
    return result
end
...
f(x) = hfree2h3d(solveforheigs(x)[:, end])[critical_point_node]*9.807*997*1e-6  #convert from head (meters) to pressure (MPa) (for water 25 C)
